...
import { X, CheckCircle2, Sparkles, ArrowRight, FileText, BadgeCheck } from 'lucide-react';
...
export const GetStartedModal: React.FC<GetStartedModalProps> = ({ isOpen, onClose }) => {
  ...
  const [role, setRole] = useState('Head of Architecture');
      ...
      >
        ...
        {submitted ? (
          <div className="text-center py-6 space-y-4">
            ...
            <h3 className="text-2xl font-bold text-[#0b1c30]">Request received</h3>
            <p className="text-sm text-[#45464d] leading-relaxed max-w-sm mx-auto">
              A founder will be in touch at <span className="font-semibold text-[#0b1c30]">{email}</span> within one working day to arrange a scoping call and tell you what we would need to assess your case.
            </p>
            <div className="pt-2">
              <button
                onClick={onClose}
          ...
          <div>
            <div className="flex items-center space-x-2.5 mb-2">
              ...
              <h2 className="text-xl font-bold text-[#0b1c30]">Book a scoping call</h2>
            </div>
            <p className="text-xs sm:text-sm text-[#45464d] mb-6">
              Tell us who you are and we will arrange a call to work out whether we can assess your case, what we would need, and how long it would take. No obligation to proceed.
            </p>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-3.5 py-2.5 rounded-lg border border-[#c6c6cd] bg-[#f8f9ff] text-sm text-[#0b1c30] focus:ring-2 focus:ring-[#4648d4]/40 focus:border-[#4648d4] outline-none"
                >
                  <option>Head of Architecture / Enterprise Architect</option>
                  <option>CIO / CTO</option>
                  <option>Transformation Director</option>
                  <option>Model Risk / Compliance</option>
                  <option>Other</option>
                </select>
              </div>
              ...
              <div className="flex items-center justify-center space-x-4 pt-2 text-[11px] text-[#76777d]">
                <span className="flex items-center"><FileText className="w-3.5 h-3.5 mr-1 text-[#4648d4]" /> Traced to your material</span>
                <span className="flex items-center"><BadgeCheck className="w-3.5 h-3.5 mr-1 text-[#15803d]" /> Signed by a named architect</span>
              </div>
            </form>
          </div>
        )}
